...
## 2.5 2026年9月 大型アップデートで完了したもの
| # | 内容 | 状態 |
|---|------|------|
| A | 基準時刻の永続化バグ修正（再訪時に次発が誤表示される問題） | ✅ 完了 |
| B | 祝日・振替休日・国民の休日の自動判定 | ✅ 完了 |
| C | Service Worker 登録の修正（オフライン動作の実現） | ✅ 完了 |
| D | ハイドレーション不整合の解消（全ページ コンソールエラー 0） | ✅ 完了 |
| E | 当日全便の一覧表示・日付切替・翌日繰り越し | ✅ 完了 |
| F | 徒歩時間から「間に合うか」を判定 | ✅ 完了 |
| G | カレンダー（.ics）書き出し | ✅ 完了 |
| H | ダークモード実装 | ✅ 完了 |
| I | データ鮮度メタ情報と未反映改定の告知 | ✅ 完了 |
| J | 時刻表データの機械検証 / ユニットテスト / ブラウザ回帰テストの CI 化 | ✅ 完了 |
| K | `/timetable/` 新設・共通レイアウト化・ページ別 OGP | ✅ 完了 |

詳細は [IMPROVEMENT_PLAN.md](IMPROVEMENT_PLAN.md) を参照。

## 3. Later (3〜6か月)
| # | Issue タイトル案 | 説明 | ラベル | 期限目安 |
|---|------------------|------|--------|----------|
| 10 | 遅延・運休情報の取得と表示（最優先の残課題） | 自治体 or SNS ソースから運行ニュースを取得し、トップに表示 | `backend`, `data`, `seo` | 6か月以内 |
...
### 3.1 次に着手すべきこと
1. **2026年1月1日改定の時刻表反映**（`meta.pendingRevision` が立っている間は告知バナーが出続けます）
2. 遅延・運休情報の取得と表示
3. 英語版（訪日観光客向け）
