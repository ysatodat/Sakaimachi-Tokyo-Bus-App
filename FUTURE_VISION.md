...
| 領域 | 状態 |
|------|------|
| UX / UI | 次発・当日全便・日付切替・徒歩時間による可否判定・カレンダー書き出しまで実装済み |
| パフォーマンス | Lighthouse Performance 0.99–1.00 / CLS 0.001–0.036 / LCP 0.8s |
| アクセシビリティ | Lighthouse Accessibility 100・Pa11y (WCAG 2.1 AA) エラー 0・ダークモード対応 |
| SEO | Lighthouse SEO 100。全便を静的 HTML で出力し、JS なしでも読める |
| PWA / オフライン | Service Worker 稼働。オフラインでも時刻表を表示できることを実機で確認済み |
| データ品質 | 改定日・出典・最終確認日をデータに保持。未反映の改定を UI で告知。CI で機械検証 |
| テスト | ユニット 29 件 + ブラウザ回帰 19 件 + Lighthouse/Pa11y を CI 必須化 |

残る最大の課題は **時刻表データそのものの鮮度**（2026年1月1日改定の反映）と
**リアルタイムな運行情報**。詳細は [IMPROVEMENT_PLAN.md](IMPROVEMENT_PLAN.md)。

## 3. 未来と現状のギャップ（2026年9月アップデート前の記録）
### 3.1 UX / UI
